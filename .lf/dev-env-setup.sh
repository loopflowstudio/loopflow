...
usage() {
  cat <<'USAGE'
Usage: .lf/dev-env-setup.sh [--check|--install] [--dry-run]

Idempotent loopflow maintainer environment setup.
Runs .lf/env-setup.sh first, then ensures maintainer tooling is available.
...
BASE_SETUP="$SCRIPT_DIR/env-setup.sh"

if [[ ! -x "$BASE_SETUP" ]]; then
  echo "Missing executable $BASE_SETUP" >&2
  exit 1
fi
...
if [[ "$MODE" == "check" && -n "$MISSING_TOOLS" ]]; then
  ...
  echo "Run: .lf/dev-env-setup.sh --install"
  exit 1
fi
