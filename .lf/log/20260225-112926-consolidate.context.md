...
- Replaced role-style builtin directions with grouped direction taxonomies:
  ...
  - `ceo.md` moved to top-level
- Added engine support for builtin direction groups generated at build time (`BUILTIN_DIRECTION_GROUPS`) and public lookup APIs.
- Added direction-group expansion in prompt assembly and fork execution paths (CLI and daemon), including dedupe and user-group override behavior.
- Updated wave plan fork flows (`wave-reduce`, `wave-polish`, `wave-expand`) to use `infra`, `ux`, `ceo`.
- Updated review/review-design/gate step prompts to use direction-driven quality coverage language.
- Removed old `roles/*` direction files.
- Added/updated tests for group expansion, nested direction lookup, flow fork direction parsing, and golden prompt parity with direction groups.
- Updated user-facing docs/examples and helper scripts to use the new taxonomy.

## Key choices

- **Group expansion happens before direction loading** so existing loading logic remains unchanged and all call sites receive concrete direction names.
- **User direction groups override builtin groups** when names collide (`.lf/directions/<group>/`).
- **Group expansion is applied in both prompt context gathering and fork planning call sites** to keep behavior consistent across local CLI and lfd execution.
- **No compatibility shim for removed role names**; docs/examples/scripts were updated to the new names instead.

## How it fits together

`build.rs` now emits a generated direction-group map from `builtins/directions/*/`. Runtime context assembly (`gather_context`) and fork planning expand configured direction names through `expand_direction_names`, which resolves repo-local group directories first, then builtin groups. Concrete direction names are then loaded through existing direction resolution, so downstream prompt formatting and execution stay unchanged.

## Risks and bottlenecks

- Any external/local automation still passing removed names (`product-engineer`, `designer`, `infra-engineer`) will fail direction lookup and must migrate.
- Direction file-name collisions across groups would still be ambiguous if two groups define the same leaf name.
- Full `cargo test --all` still has two environment-dependent Docker test failures when `/var/run/docker.sock` is unavailable.

## What's not included

- No backward-compatibility aliasing from old role names to new taxonomy.
- No semantic change to flow execution/planning beyond direction name expansion.
- No changes to wave queue content under `wave/`.
