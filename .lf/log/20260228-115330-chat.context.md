...
<lf:docs>
<lf:jack-heart.qawave.20260228_1113>
# QA/Deploy Wave: Branch Construct + Looping Waves

## What to build

Three new primitives that together enable a daily QA → fix → deploy cycle:
1. **`branch` flow construct** — a step in a flow that runs a decision step and routes to sub-flows based on its output
2. **`deploy_or_fix` step** — built-in step that reads triage output and decides: fix or deploy
3. **`until: finished` wave scheduling** — cron-triggered wave that loops (new WaveRuns each iteration) until the cycle completes

## Depends on

`jack-heart.chords.20260227_1641` — WaveMode (Loop | Cron | Manual), FlowRun, signal cleanup. This branch should rebase onto that once it lands.

## The Use Case

One thorough QA session per day instead of shallow checks on every change. Review/gate can be lighter during normal flow because the daily QA is the real quality gate.

> "i would like to have one REALLY GOOD qa session a day rather than trying to do it 5 times an hour"
...
steps:
  ...
  - branch:
      step: deploy_or_fix
      paths:
        fix: qa-fix        # fix flow, then new WaveRun loops back to qa
        deploy: deploy     # ship it, cycle complete
```

1. Cron fires at 9am → WaveRun 1: qa → triage → deploy_or_fix says "fix"
2. WaveRun 2: qa-fix flow (implement → compress → lint → gate → update-wave)
3. WaveRun 3: qa → triage → deploy_or_fix says "deploy"
4. WaveRun 4: deploy → cycle finished, stop looping

Each WaveRun = one branch, at most one PR. All findings, verdicts, and routing decisions tracked in scratch/ and committed.

The fix flow is a slight wrapper around build — no review step (next QA pass acts as review), but keeps update-wave to track what was fixed.

## Primitive 1: `branch` flow construct

A new flow action type alongside existing `step` and `fork`.
...
steps:
  ...
  - triage
  - branch:
      step: deploy_or_fix      # built-in step that makes the decision
      paths:                    # map decision → sub-flow
        fix: qa-fix
        deploy: deploy
```
...
1. Flow runner hits a `branch` action
2. Runs the named step (deploy_or_fix) as a normal agent step
3. Step writes its decision and reasoning to scratch/
4. Flow runner reads the decision from scratch/ (structured header + natural language reasoning)
5. Maps decision to a path, runs that sub-flow

### Decision format

The decision step writes to `scratch/route-<step-name>.md`:
...
Triage found 2 blocking issues: the auth token refresh race condition
and the missing error handler on websocket disconnect. Both are
regressions from yesterday's merge. Polish items (button alignment
on settings page, inconsistent loading spinner) written to wave/polish/.
```

First line is `path: <key>` matching one of the `paths:` in the flow definition. Everything after is reasoning — human-readable, git-tracked, available for the next step to read.

### Why files

> Files work because of sequencing but also because they are inherently tracked and transferrable.

- Sequential steps → no race conditions
- Git tracked → full audit trail of every routing decision
- Transferable → human can read, edit, and re-run
...
#[derive(Debug, Clone)]
pub enum FlowAction {
    Step(String),                    // existing
    Fork(ForkAction),               // existing
    Branch(BranchAction),           // new
}

#[derive(Debug, Clone)]
pub struct BranchAction {
    pub step: String,                // step to run for decision
    pub paths: HashMap<String, String>,  // decision key → flow name
}
```

Flow parser reads `branch:` blocks from YAML. Executor runs the step, reads `scratch/route-<step>.md`, extracts the `path:` line, looks up the flow, and runs it.

## Primitive 2: `deploy_or_fix` step

Built-in step in `.lf/steps/`. Reads triage output from scratch/, decides whether the current state is deployable or needs fixes.

The step doesn't know about flow branching. It just reads the assessment and writes a decision. The `branch` construct reads its output.

```markdown
# deploy_or_fix

Read the triage assessment in scratch/. Decide: are there blocking issues
that must be fixed before deploy, or is the current state clean enough to ship?

Write blocking issues that need fixing as a design doc to scratch/ for the
fix flow to consume. Write polish items to wave/polish/ backlog.

## Output

Write to scratch/route-deploy_or_fix.md:
- First line: `path: fix` or `path: deploy`
- Then: reasoning, blocking issues summary, what was sent to polish backlog
```

### Composability

> "i would like to do it in a way thats more composable and doesnt quite require triage to know its being run in a branch"

- `triage` writes a rich natural language assessment — it doesn't know about routing
- `deploy_or_fix` reads the assessment and makes a routing decision — it doesn't know about triage specifically, it reads whatever is in scratch/
- The `branch` construct maps the decision to flows — it doesn't know about QA or deploy

Each piece is reusable. A different flow could use `triage` without branching. A different branch could use a different decision step. The `branch` construct works with any step that writes a `path:` line.

## Primitive 3: `until: finished` wave scheduling

Builds on the chords branch's WaveMode (Loop | Cron | Manual).

Today: cron waves fire once per schedule tick. Loop waves loop forever.

New: a cron wave with `until: finished` starts on schedule, then keeps creating new WaveRuns until the cycle completes.

```yaml
stimulus:
  kind: cron
  cron: "0 9 * * *"
  until: finished      # loop until cycle completes
```

### Wave cycle state

The wave carries cycle state:

```rust
pub enum CycleState {
    Idle,        // waiting for next cron trigger
    Active,      // in a QA/fix/deploy cycle
    Finished,    // cycle complete, return to Idle on next tick
}
```

- Cron fires → state becomes `Active`
- Each WaveRun completes → scheduler checks: did the flow signal "finished"?
- If not finished → create another WaveRun with the next flow (determined by branch outcome)
- If finished → state becomes `Finished`, stop looping

### How "finished" is signaled

The deploy flow's last step completing signals the cycle is done. The `branch` construct's path selection determines which flow runs next:
- `fix` path → after qa-fix completes, loop back to qa-deploy flow (new WaveRun)
- `deploy` path → after deploy completes, cycle is finished

### Determining next flow on loop

When a WaveRun completes and the cycle is still active:
- If the WaveRun ran a flow that was selected via `branch`, and the branch was in a parent flow, the parent flow resumes from the branch point
- Practically: after the `fix` path completes, the wave re-runs the top-level `qa-deploy` flow from the beginning (new WaveRun, fresh QA)

### Safety valve

Max iterations per cycle (configurable, default 5). If QA keeps finding issues after N fix rounds, the cycle pauses and alerts. Prevents infinite loops.
...
> "technically i think theres a slight wrapper around build. i dont think we need review because we can do review in the next QA step, but maybe update-wave"
...
Standard build flow. No review step — the next QA pass serves as review. Keeps update-wave to track what was fixed in wave state.

## Polish backlog flow

QA triage produces two outputs:
1. Blocking issues → stay in scratch/ for the fix flow
2. Polish items → written to `wave/polish/` backlog

This creates a natural flow between "blocks deploy" and "track for later." The polish wave is a separate wave that can be worked on independently.

## Constraints

- **Depends on chords branch** — WaveMode, FlowRun, signal cleanup must land first
- **One WaveRun = one branch = at most one PR** — the loop creates new WaveRuns, not multiple PRs per WaveRun
- **Decision step output format** — `path: <key>` as first line is the contract between decision steps and the branch construct
- **File-based verdicts** — not in-memory state, not API calls. Files in scratch/, committed to git.

## Out of scope

- Concerto UI for QA cycle visualization (future)
- Generic routing DSL (deploy_or_fix is purpose-built, not a universal router)
- Human-in-the-loop gates in Concerto (the daily QA is autonomous; human intervention is editing scratch/ and re-running)
- The QA step prompt itself (content of what "thorough QA" means — that's a step to write, not architecture)

## Done when

```bash
# branch construct parses and executes
cargo test --all

# flow with branch runs deploy_or_fix step, reads verdict, routes correctly
# wave with until:finished loops WaveRuns and stops when cycle completes
uv run pytest python/tests/
```

Observable:
- A flow YAML with `branch:` parses without error
- The branch construct runs the named step, reads `scratch/route-<step>.md`, and routes to the correct sub-flow
- A wave with `until: finished` creates sequential WaveRuns until the cycle completes
- Max iterations safety valve stops runaway loops
- deploy_or_fix step reads triage output and produces a `path:` verdict
