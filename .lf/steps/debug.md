...
## Headless + no input → exit cleanly

If the run is headless **and** there is no clipboard content and no explicit error target, do not enter an ambiguous state. Print a one-line message and exit without making changes:

> `debug: no error provided (headless run, empty clipboard, no target). Re-invoke with 'lf debug -c' after copying a stacktrace, or pass the error inline.`

Do not ask questions, do not modify code, do not write scratch/ artifacts. The absence of an error is itself the answer.

## Interactive + no input → ask

If interactive and clipboard is empty or no -c flag, ask what error to debug.
