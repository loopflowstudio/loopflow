...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled by setting `skip_eng_review: true` in `.lf/config.yaml` (the "don't bother me" setting).
...
Check diff size: `git diff <base>...HEAD --stat | tail -1`. If the diff is >200 lines, add: "Note: This is a large diff. Consider running `gstack:eng-review` or `gstack:autoplan` for architecture-level review before shipping."

If CEO Review is missing, mention as informational ("CEO Review not run — recommended for product changes") but do NOT block.

For Design Review: check if the diff touches frontend files (`git diff origin/<base> --name-only | grep -qE '\.(tsx?|jsx?|css|scss|sass|html|svelte|vue)$'`). If it does and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running gstack:design-audit for a full visual audit post-implementation." Still never block.
...
Write to `scratch/$BRANCH.md` (appending a test plan section, or to `.gstack/qa-reports/test-plan.md` if scratch is not appropriate):
...
# Primary: loopflow design doc
PLAN=""
[ -f "scratch/${BRANCH}.md" ] && PLAN="scratch/${BRANCH}.md"
# Fallback: legacy plan file locations
if [ -z "$PLAN" ]; then
  for PLAN_DIR in "$HOME/.claude/plans" "$HOME/.codex/plans"; do
    [ -d "$PLAN_DIR" ] || continue
    PLAN=$(find "$PLAN_DIR" -name '*.md' -newer "$PLAN_DIR" -maxdepth 1 2>/dev/null | head -1)
    [ -n "$PLAN" ] && break
  done
fi
...
Check if the diff touches frontend files. Scan the changed file extensions:

```bash
git diff origin/<base> --name-only | grep -qE '\.(tsx?|jsx?|css|scss|sass|html|svelte|vue)$' && echo "SCOPE_FRONTEND=true" || echo "SCOPE_FRONTEND=false"
SCOPE_FRONTEND=$(git diff origin/<base> --name-only | grep -qE '\.(tsx?|jsx?|css|scss|sass|html|svelte|vue)$' && echo true || echo false)
```
...
```bash
mkdir -p .gstack
echo '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}' >> .gstack/reviews.jsonl
```
...
```bash
mkdir -p .gstack
echo '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}' >> .gstack/reviews.jsonl
```
...
which codex 2>/dev/null && echo "CODEX_AVAILABLE" || echo "CODEX_NOT_AVAILABLE"
# Respect opt-out setting from .lf/config.yaml
OLD_CFG=$(grep 'codex_reviews' .lf/config.yaml 2>/dev/null | grep -o 'disabled' || true)
...
```bash
mkdir -p .gstack
echo '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"medium","commit":"'"$(git rev-parse --short HEAD)"'"}' >> .gstack/reviews.jsonl
```
...
```bash
mkdir -p .gstack
echo '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"large","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}' >> .gstack/reviews.jsonl
```
