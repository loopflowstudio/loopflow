...
The output is both immediate (updated docs) and strategic (a prioritized list of polish projects with compelling evidence). This strategic output feeds into roadmap planning—it's one way to generate project plans grounded in real friction.
...
The evidence section is the key. Specific examples make prioritization possible. "Error messages are bad" is not actionable. "Running `lf foo` with no args shows 'Error: None' with no explanation" is.
