...
---
Pick the ONE Concerto behavior this loop will try to enable, and the specific
...
- `scratch/ux-research/design-guidelines.md` — what we already believe about
  Concerto's UX. Don't re-litigate settled beliefs; build on them.
- `scratch/ux-research/personas.md` — who we're designing for.

React to the **actual current UI**, not a blank slate:

- `swift/Concerto/Platform/macOS/Views/RepoSidebarWindow.swift` — the shipped
  reset surface (repo sidebar filtering a wave list; rows display-only).
- `swift/Concerto/Platform/macOS/Views/ContentView.swift` — the slice-1 list.
...
bar (e.g. "a developer with several active waves opens Concerto and within
