...
#   cp .env ~/loopflow/docker/.env  # (with LFD_AUTH_TOKEN set)
#
# Prerequisites:
...
scp docker/.env "$HOST":~/loopflow/docker/.env
ssh "$HOST" 'cd ~/loopflow && docker compose -f docker/docker-compose.yml -f deploy/docker-compose.prod.yml up -d --build'
ssh "$HOST" 'cd ~/loopflow && docker compose -f docker/docker-compose.yml -f deploy/docker-compose.prod.yml ps'
