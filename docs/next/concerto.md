...
![Concerto main window](../concerto-main.png)
...
```bash
cd Concerto
xcodebuild -scheme Concerto -configuration Release
```
...
Click **Run** or press **⌘↵** to launch. If you're on the main branch, Concerto creates a new worktree automatically with a generated name.
...
Concerto reads `.lf/config.yaml` from the open repository. Key settings:
