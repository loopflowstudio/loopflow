...
## 2026-06-29 — Asana is the only PM provider; rip out the provider abstraction

**Context:** v0.9.10 shipped three PM providers — Asana, Linear, Notion — behind a `PmProviderKind` enum and a `PmProvider` trait. After running all three, Asana is the keeper. The multi-provider machinery was complexity serving hypothetical other users; loopflow is being narrowed to target its one real user. The stale PR #659 ("canonical Asana wave set; drop Linear and Notion") had the right instinct on Asana-only but bundled it with a canonical-wave-from-Asana model that the 2026-06-29 Portfolio-tiers direction superseded (project ordering is local Concerto state, not PM-backed). So #659 is closed unmerged and the Asana-only cut is redone clean.

**Decision:** Asana is the sole PM provider. Delete Linear and Notion entirely — clients, auth flows, CLI commands, config fields, the live priority example. Rip out the *abstraction*, not just the implementations: the `PmProviderKind` enum and the `PmProvider` trait both go. `AsanaClient` is used concretely throughout; frontmatter stores `asana_id` directly with no `id_for(provider)` dispatch; `provider:` parameters disappear from the ops surface.

**Implications:** Re-adding a provider later is a real rewrite, not a config flip — accepted, because the code is cheap to rewrite and the abstraction earned nothing while only Asana was live. The canonical-wave-discovery ideas from #659 (`/v0/waves/discovered`, `roadmap_poller`, `repos:` config override, `ship-roadmap` flow) are dropped on the floor, not salvaged. No compatibility shim for `pm.provider: linear|notion` in existing configs — those configs are internal, and per repo policy we migrate rather than carry both. **Test-coverage note:** removing the `PmProvider` trait removed the seam the `ops/pm.rs` orchestration tests (three-way sync, export create/update/skip, bootstrap reverse-order) mocked against. Those tests were dropped, not rewritten onto HTTP fixtures — `AsanaClient`'s own `test_server` tests in `asana.rs` cover the client layer, and the pure pm.rs helpers keep their unit tests, but the orchestration glue is now uncovered. Re-adding HTTP-fixture coverage for it is a worthwhile follow-up.
