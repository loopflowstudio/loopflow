...
At tag time, `lf op release` renames this directory to `release/v<version>/` and feeds this file into the release-notes step as primary source material.
...
**Decision:** Add `release/<version>/DECISIONS.md` as an append-only ledger that interactive runs update as decisions are made. At tag time, `lf op release` renames `release/unreleased/` to `release/v<version>/`, archives `RELEASE_NOTES.md` to `release/v<version>/NOTES.md`, and passes `DECISIONS.md` into the release-notes step as primary input.
