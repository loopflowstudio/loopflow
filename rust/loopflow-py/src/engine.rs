...
use loopflow_engine::agent::{launch_agent, LaunchConfig, LaunchResult as RustLaunchResult};
use loopflow_engine::config::{load_config_or_default, parse_model};
...
impl PyPromptComponents {
    ...
    fn token_count(&self) -> usize {
        loopflow_engine::prompt::analyze_tokens(&self.inner)
    }
...
pub fn register(parent: &Bound<PyModule>) -> PyResult<()> {
    let engine_mod = PyModule::new_bound(parent.py(), "engine")?;

    engine_mod.add_class::<PyLaunchResult>()?;
    engine_mod.add_class::<PyPromptComponents>()?;
    engine_mod.add_class::<PyRebaseResult>()?;
    engine_mod.add_class::<PyBranchInfo>()?;
    engine_mod.add_class::<PyLandResult>()?;

    engine_mod.add_function(wrap_pyfunction!(py_count_tokens, &engine_mod)?)?;
    engine_mod.add_function(wrap_pyfunction!(py_parse_model, &engine_mod)?)?;
    engine_mod.add_function(wrap_pyfunction!(py_gather_context, &engine_mod)?)?;
    engine_mod.add_function(wrap_pyfunction!(py_launch_agent, &engine_mod)?)?;
    engine_mod.add_function(wrap_pyfunction!(run_step, &engine_mod)?)?;

    let git_mod = PyModule::new_bound(parent.py(), "git")?;
    ...
    engine_mod.add_submodule(&git_mod)?;

    parent.add_submodule(&engine_mod)?;
    Ok(())
}
