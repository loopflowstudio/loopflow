...
//! ([`crate::controller::wave::runner`]), the rendered GOAL.md seed — and NOTHING
//! pen-shaped: it never touches a journal file. Its two connections to the
//! listener are the whole protocol (see [`crate::controller::wave::wire`]):
//!
...
use crate::controller::wave::runner::{path_for_children, run_loop, LoopConfig};
use crate::controller::wave::runtime::InboxItem;
use crate::controller::wave::server;
use crate::controller::wave::subscription::stream_events;
...
use crate::controller::wave::WAVE_SERVER_ENDPOINT_ENV;
...
pub fn run(name: &str) -> Result<()> {
    ...
    let loop_cwd = crate::controller::wave::resolve_resident_worktree(&main_repo, &wave)?.path;
...
mod tests {
    ...
    async fn refused_listener_call_is_fatal_not_retried() {
        ...
        let observer = std::sync::Arc::new(crate::controller::wave::registry::ObserverSlot::new(
            runtime.clone(),
            None,
        ));
    ...
    fn inbox_frames_map_to_inbox_items() {
        let message = inbox_item(InboxFrame::Message {
            id: "msg-3".into(),
            op: crate::controller::wave::journal::MessageOp::Steer,
            text: "focus".into(),
            source: None,
        });
        ...
        assert_eq!(
            message.op,
            crate::controller::wave::journal::MessageOp::Steer
        );
    ...
    fn wave_resident_reuses_its_worktree_without_claiming_dirty_main_state() {
        ...
        let first =
            crate::controller::wave::resolve_resident_worktree(repo.path(), "infrastructure")
                .expect("clean main");
        assert_ne!(
