...
/// List of all built-in step names (canonical, user-facing).
///
/// Namespaced steps (e.g., "garden/scan") shadow raw file stems (e.g., "scan")
/// auto-generated by build.rs. This returns only the canonical form.
pub fn builtin_step_names() -> Vec<&'static str> {
    // Collect raw stems that are shadowed by a namespaced entry.
    let namespaced_stems: std::collections::HashSet<&str> = NAMESPACED_STEPS
        .keys()
        .filter_map(|k| k.rsplit('/').next())
        .collect();

    let mut names: Vec<_> = BUILTIN_STEPS
        .keys()
        .copied()
        .filter(|k| !namespaced_stems.contains(k))
        .collect();
