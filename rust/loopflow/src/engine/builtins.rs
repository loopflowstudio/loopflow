...
/// Resolve a name to its builtin step key.
pub fn resolve_builtin_step(name: &str) -> Option<&'static str> {
    BUILTIN_STEPS.get_key_value(name).map(|(key, _)| *key)
}

/// Resolve a name to its builtin flow key.
pub fn resolve_builtin_flow(name: &str) -> Option<&'static str> {
    BUILTIN_FLOWS.get_key_value(name).map(|(key, _)| *key)
}

/// Resolve a name to its builtin goal key.
pub fn resolve_builtin_goal(name: &str) -> Option<&'static str> {
    BUILTIN_GOALS.get_key_value(name).map(|(key, _)| *key)
}
