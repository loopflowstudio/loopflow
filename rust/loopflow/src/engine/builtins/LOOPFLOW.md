...
Use orchestration only when the human or selected skill calls for it. Do not
...
lf pr submit                         # prepare for a human's merge click
...
for a human to land; arm/land request auto-merge. Bare land keeps the Task open;
...
the review page; use it only when the human asks to see the PR.
...
operations unless already authorized by the human or selected workflow.
...
follow its authored order and human review boundaries.

## Speak and inspect

Answer the present human in this conversation. Never open another human
session merely to reach them. Headless, `lf ask "<request>"` opens a durable
human session and waits for completion. Respect existing authorization.
