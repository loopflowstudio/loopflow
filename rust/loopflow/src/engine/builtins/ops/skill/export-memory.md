...
3. Compile the memory:
   ...
   - Keep it tight enough to seed the next loop directly.
   - Organize into typed blocks — short `##` sections, each one kind of
     ...
     - **Glossary** — domain terms, names, and coinages the next loop needs.
     - **How To** — procedures and recipes: how to run, build, test, deploy, or
       debug the thing, distilled so the next loop doesn't re-derive them.
...
- Do not restart the wave or resident loop.
