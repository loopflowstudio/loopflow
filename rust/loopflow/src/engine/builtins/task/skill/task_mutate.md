...
- If review has an obvious fix, make the bounded repair in this Task worktree,
  verify it, and update the same PR. `lf pr land` owns post-arm CI repair.
...
Return concise evidence and the next external wait or repair. Write no loop bit.
