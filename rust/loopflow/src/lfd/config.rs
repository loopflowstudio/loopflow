...
///   (1 hour by default, or longer for explicit pairing tokens) or revocation. lfd mints tokens and can send them to studio,
///   which distributes them to mobile clients. Without studio credentials,
///   the same local ledger still supports accountless pairing. Validation is local (no
