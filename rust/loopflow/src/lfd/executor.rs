...
use crate::engine::git::{
    commit, create_branch, current_branch, hash_areas, is_clean, push_with_upstream, stage_all,
};
...
use crate::lfd::types::{
    Agent, AgentStatus, Event, SidecarKind, StimulusKind, Summary, Wave, WaveRun, WaveRunKind,
...
impl WaveExecutor {
    ...
    async fn execute_ci_fix_agent(
        ...
        self.event_hub
            .send(Event::agent_ended(agent_id, agent_status));

        if exit_code != 0 {
            return Err(anyhow!(
                "CI fix debug step failed with exit code {exit_code}"
            ));
        }
...
fn ci_fix_worktree_path(repo: &Path, wave_name: &str, run_id: &LfdId) -> PathBuf {
    let repo_root =
...
fn cleanup_ci_fix_worktree(worktree: &Path) -> Result<()> {
    if !worktree.exists() {
        return Ok(());
    }
...
fn commit_and_push_ci_fix(worktree: &Path, target_branch: &str, check_name: &str) -> Result<()> {
...
fn is_active_wave_run_status(status: WaveRunStatus) -> bool {
    matches!(
        status,
...
fn is_ephemeral_worktree_path(path: &str) -> bool {
    path.contains("-fork-") || path.contains(".ci-fix.")
}

fn format_ci_failure_message(failure: &CiFailure) -> String {
    format!(
