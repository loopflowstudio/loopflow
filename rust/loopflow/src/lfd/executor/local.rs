...
impl AgentExecutor for LocalProcessExecutor {
    async fn run(&self, cmd: Vec<String>, cwd: &Path, context: ExecutionContext) -> Result<i32> {
        ...
        for env_name in crate::provider_auth::api_key_env_names() {
            if !crate::provider_auth::api_key_env_allowed_for_program(program, env_name) {
                command.env_remove(env_name);
            }
        }

        // Inject DB-backed provider tokens as env vars.
        for (key, value) in crate::provider_auth::provider_env_vars(&self.store).await {
            if !crate::provider_auth::provider_env_allowed_for_program(program, &key) {
                continue;
            }
