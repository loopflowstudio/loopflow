...
const QUERY_COUNT: usize = Query::DeleteChatMemoryBlock as usize + 1;
...
const QUERY_DEFS: [QueryDef; QUERY_COUNT] = [
    ...
    QueryDef {
        template: "SELECT id, wave_id, iteration, step_index, status, worktree, branch,\n                    started_at, ended_at, error, snapshot_repo, snapshot_flow, snapshot_direction,\n                    snapshot_area, snapshot_pr, flow_parents, activation_log_id, run_kind, sidecar_kind,\n                    parent_run_id, parent_pr_number, stack_position, stack_group_id, stack_status,\n                    lineage_inferred\n             FROM wave_runs\n             WHERE wave_id = {p1} AND status IN ({p2}, {p3}, {p4}) AND run_kind = {p5}\n             ORDER BY started_at DESC LIMIT 1",
        sqlite_override: None,
        postgres_override: Some(
            "SELECT id, wave_id, iteration, step_index, status, worktree, branch,\n                    started_at, ended_at, error, snapshot_repo, snapshot_flow, snapshot_direction,\n                    snapshot_area, snapshot_pr, flow_parents, activation_log_id, run_kind, sidecar_kind,\n                    parent_run_id, parent_pr_number, stack_position, stack_group_id, stack_status,\n                    lineage_inferred\n             FROM wave_runs\n             WHERE wave_id = {p1} AND status = ANY({p2}) AND run_kind = {p3}\n             ORDER BY started_at DESC LIMIT 1",
        ),
    },
    QueryDef {
        template: "SELECT id, wave_id, iteration, step_index, status, worktree, branch,\n                    started_at, ended_at, error, snapshot_repo, snapshot_flow, snapshot_direction,\n                    snapshot_area, snapshot_pr, flow_parents, activation_log_id, run_kind, sidecar_kind,\n                    parent_run_id, parent_pr_number, stack_position, stack_group_id, stack_status,\n                    lineage_inferred\n             FROM wave_runs WHERE wave_id = {p1} AND run_kind = {p2}\n             ORDER BY started_at DESC LIMIT 1",
        sqlite_override: None,
        postgres_override: None,
    },
    QueryDef {
    ...
    QueryDef {
        template: "SELECT id, wave_id, kind, cron, last_main_sha, last_triggered_at, created_at, enabled, source_wave_id\n             FROM stimuli ORDER BY created_at",
        sqlite_override: None,
        postgres_override: None,
    },
    QueryDef {
    ...
    QueryDef {
        template: "SELECT id, wave_id, kind, cron, last_main_sha, last_triggered_at, created_at, enabled, source_wave_id\n             FROM stimuli WHERE kind = {p1} ORDER BY created_at",
        sqlite_override: None,
        postgres_override: None,
    },
    QueryDef {
    ...
    QueryDef {
        template: "INSERT INTO stimuli (id, wave_id, kind, cron, last_main_sha, last_triggered_at, created_at, enabled, source_wave_id)\n             VALUES ({p1}, {p2}, {p3}, {p4}, {p5}, {p6}, {p7}, {p8}, {p9})",
        sqlite_override: None,
        postgres_override: None,
    },
    QueryDef {
        template: "UPDATE stimuli SET\n                kind = {p1}, cron = {p2}, last_main_sha = {p3},\n                last_triggered_at = {p4}, enabled = {p5}, source_wave_id = {p6}\n             WHERE id = {p7}",
        sqlite_override: None,
        postgres_override: None,
    },
    QueryDef {
    ...
    QueryDef {
        template: "SELECT id, wave_id, stimulus_id, source, reason, from_sha, to_sha, queued_at\n             FROM pending_activations WHERE wave_id = {p1} ORDER BY queued_at",
        sqlite_override: None,
        postgres_override: None,
    },
    QueryDef {
        template: "INSERT INTO pending_activations (id, wave_id, stimulus_id, source, reason, from_sha, to_sha, queued_at)\n             VALUES ({p1}, {p2}, {p3}, {p4}, {p5}, {p6}, {p7}, {p8})",
        sqlite_override: None,
        postgres_override: None,
    },
    QueryDef {
