...
async fn run_project_inner(store: SharedStore, project_id: ProjectId) -> Result<()> {
    'launch: loop {
        ...
        store.update_project_progress(&project).await?;
        store
        ...
        if let Err(error) = store.update_project_progress(&project).await {
...
async fn handle_attachment(store: &SharedStore, project: &Project, line: String) -> Result<()> {
    let line = line.trim();
    if line.is_empty() {
        return Ok(());
    }
...
mod tests {
    ...
    async fn project_fixture() -> (tempfile::TempDir, PathBuf, SharedStore, Project) {
        ...
        (directory, database, store, project)
    }
    ...
    async fn project_plan_refresh_reaches_the_next_boundary_once_with_all_krs() {
        ...
        store.update_project_progress(&project).await.unwrap();
