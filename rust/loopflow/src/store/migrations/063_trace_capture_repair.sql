-- 062 was recorded after stopping at its first CREATE INDEX collision.
-- The original capture rows remain in the renamed 061 tables; rebuild the
-- target contract from those tables and then remove the stranded schema.
...
DROP INDEX idx_agent_turns_started;
DROP INDEX idx_context_assets_kind;
DROP INDEX idx_context_assets_hash;
DROP INDEX idx_context_decisions_decision;
