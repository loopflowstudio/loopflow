-- Frozen snapshot of a loopflow store at the 0.10 release generation.
--
-- This fixture is deliberately INDEPENDENT of the current MIGRATIONS registry:
-- it is the product schema and ledger a real 0.10 database carried, captured
-- once and never regenerated. The upgrade test loads it and runs the current
-- canonical tail over it, so a rewritten early migration diverges the recomputed
-- schema from this frozen one and fails the test — which a registry-derived
-- prior generation (a prefix of MIGRATIONS) can never catch.
--
-- Seeded data (two waves in a parent/child foreign-key relationship, one
-- provider token) proves live rows and referential integrity survive every
-- table rebuild in the 0.11 tail. Do not edit by hand.
...
, lf_bin TEXT, db_path TEXT, lf_home TEXT, abandon_requested_at INTEGER, abandon_reason TEXT);
...
INSERT INTO "provider_tokens" VALUES('github','tok',NULL,NULL,NULL,'octocat',1700000000,'oauth',0);
...
INSERT INTO "schema_migrations" VALUES('0.10.001_initial',1700000000);
INSERT INTO "schema_migrations" VALUES('0.10.002_session_execution_context',1700000000);
...
INSERT INTO "trace_capture_meta" VALUES(1,1784336508);
CREATE TABLE waves (
    id TEXT PRIMARY KEY,
...
INSERT INTO "waves" VALUES('wave-root','infrastructure','loopflow',1700000000,NULL);
INSERT INTO "waves" VALUES('wave-child','developer-efficiency','loopflow',1700000000,'wave-root');
