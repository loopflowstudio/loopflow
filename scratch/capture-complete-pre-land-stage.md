...
`duration_seconds`. Selected work begins as `not_run`, `executed=false`, and a
measured duration of zero. A started stage always finishes with a measured
partial or complete duration, including interruption. The top-level
...
receipt rather than a late reconstruction. `SIGINT` and `SIGTERM` become a typed
interruption: the active process group is stopped and reaped, the active stage
receives its partial duration, later selected stages remain explicitly
`not_run`, postflight resource measurement is attempted, and the receipt is
finished before the conventional signal exit code is returned. Ordinary phase
failures, resource blocks, suite pre/postcheck failures, and unexpected runner
exceptions use the same finish path. An unexpected exception is re-raised only
after the failed receipt is durable.

The writer keeps the current temp-file + `os.replace` protocol, adds an fsync of
the parent directory after the rename, and refuses a second terminal write.
Running checkpoints remain useful crash evidence. The scorecard never converts
...
For attribution, add one schema-4 receipt reference to the existing Task PR.
The reference is set in the same transaction as its exact-head merge request
and becomes immutable once that landing merges. The migration adds:
...
- `task_prs.preland_tracking_complete INTEGER NOT NULL DEFAULT 0`;
- a check that tracked landings have a receipt reference; and
- a partial unique index on `preland_receipt_ref`, preventing one receipt from
  satisfying two Task PRs; plus an update trigger preserving the merged
  landing's reference and tracking disposition.
...
repository, an initial managed Task `arm` or `land` resolves the latest passed
schema-4 handoff for the current worktree before its first remote merge
mutation. The resolver is owned by `scripts/test.py`, reuses the existing
tree/plan fingerprint rules, and ignores `scratch/` handoff files so the gate
review and PR copy do not invalidate the tested product tree. It emits only the
relative receipt reference. The operation validates and passes that reference
to `request_task_pr_merge`; the store writes the exact-head merge request and
its evidence reference in the same transaction before `finalize_remote` can arm
GitHub. `pr_landings` reads the Task PR during supervision but does not copy the
...
interrupted, already-consumed, or content-mismatched receipt is not eligible and
produces the exact rerun command. Budget values never decide whether a receipt
is eligible.
...
the Task PR merges, neither the reference nor its tracking disposition can
change.

Repositories that do not declare this structured protocol and direct non-Task
landings remain explicit unsupported populations with
`preland_tracking_complete=0`; they are outside this repository-owned Project
population. Managed Task `submit` is already rejected. Existing historical Task
PRs also remain untracked rather than receiving invented links.
...
- A required stage is eligible once it appears in a terminal schema-4 plan.
  Its status and duration make it measured even when it is `not_run`.
- Stage p50/p95 use only `executed=true` durations. An observed `not_run` proves
  coverage but never makes the stage look artificially fast.
...
- A landing is eligible once a supported repository's Task PR receives an auto
  ...
  settlement boundary, not the tracking bit, define eligibility, so a buggy
  writer cannot hide a missing link by writing
  `preland_tracking_complete=0`. The landing is measured only when its unique
  reference resolves to a passed terminal schema-4 receipt.
...
| Why do failed plans lower phase coverage? | `phase_rows` counts every planned phase as eligible but drops `not_run` from values, and `measured_row` equates numeric samples with evidence coverage. | Count explicit stage results as measured; calculate latency only from executed samples and publish `sampled`. |
...
- In scope: schema-4 gate receipt types and validation in `scripts/test.py`;
  ...
  receipt resolution for supported repositories; durable linkage on managed
  Task PR settlement requests made by `lf pr arm`/`land`; scorecard
  coverage/sample separation; deterministic
  Python and release-equivalent Rust fixtures; user documentation.
- Out of scope: hosted CI job telemetry; treating `lf pr arm` or `lf pr submit`
  as watched landings; inventing a generic test-plan protocol for repositories
  that do not declare structured support; backfilling schema 1-3 records or
  historical landings; changing build/CPU budgets; deleting or relocating
  source, worktrees, durable state, receipts, traces, or caches outside the
  existing recovery allowlist.
...
- A release-equivalent Rust store fixture applies the materialized migration,
  creates tracked Task PR merge requests with receipt references, rejects a
  tracked request without one, and rejects reuse of one reference by another
  Task PR.
...
- A scorecard fixture proves historical/untracked landing rows remain named
...
2. Add supported-repository receipt resolution plus the immutable Task PR merge
   request reference/tracking migration and release-equivalent store proofs.
