...
in flight — armed to auto-merge, blocked only on a queued swift-test runner**).
...
1. **PR4 (this) — Project/KR claim provenance via a PM overlay + `lf pm cite`.**
2. Report provenance + automatic receipts — authored Wave reports (chat turns)
   carry the receipts for what they summarized, attached at authoring.
3. Correction without DB edits — re-cite/retract through an authored API only.
4. Shared Mac/iOS/chat drill-down over the resolver.
5. Survival — receipts keep resolving across land / restart / successors / migrations.
6. Real N/N orphan-free dogfood — `lf doctor` receipts clean on a live wave for a week.
