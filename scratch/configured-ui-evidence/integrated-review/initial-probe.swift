...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: "/tmp/loo291-integration/Loopflow Integration.app"), configuration: configuration) { running, error in
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
        ...
        do {
            ...
            let launch = try find(root, id: "sessions-new-conversation")
            try check((text(launch, "AXTitle") + text(launch, "AXDescription")).contains("LOO-291"), "Conversation affordance names selected Task")
