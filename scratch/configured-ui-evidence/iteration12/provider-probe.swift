...
setbuf(stdout, nil)
let proofRoot = "/tmp/loo291-iteration12/result"
...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: "/tmp/loo291-iteration12/Loopflow Trial.app"), configuration: configuration) { running, error in
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
        ...
        do {
            ...
            print("System focused PID", focusedPID(), "owned", running.processIdentifier)
            ...
            _ = try find(root, id: taskId)
            try press(root,title:"All work")
            try press(root,title:"New terminal")
            settle(1)
            try send("printf '%s\\n' $$ > /tmp/loo291-iteration12/result/main-shell.pid",enter:true)
            ...
            try press(root,title:"New terminal")
            settle(1)
            try send("printf '%s\\n' $$ > /tmp/loo291-iteration12/result/task-companion.pid",enter:true)
            try press(root,title:"Split worktrees right")
            guard let picker=elements(root).first(where:{["AXMenuButton", "AXPopUpButton"].contains(text($0,"AXRole")) && (text($0,"AXDescription")=="Choose worktree" || text($0,"AXTitle")=="Choose worktree")}) else { throw Failure(message:"Missing outer picker") }
            ...
            try press(root,title:"New terminal")
            settle(1)
            try send("printf '%s\\n' $$ > /tmp/loo291-iteration12/result/main-companion.pid",enter:true)
            ...
            let sentence="Do not use tools or edit files. Reply only LOO291_ITERATION12_ROW_RETURN."
            ...
            try send("Reply only LOO291_ITERATION12_ROW_RETURN.",enter:true)
            let replyDeadline=ProcessInfo.processInfo.systemUptime+60
            while try !messages("assistant").contains("LOO291_ITERATION12_ROW_RETURN") && ProcessInfo.processInfo.systemUptime<replyDeadline { settle(0.3) }
            let user=try messages("user").last ?? ""
            let answered=try messages("assistant").contains("LOO291_ITERATION12_ROW_RETURN")
            ...
            try send("printf 'LOO291_SHELL_RETURNED\\n' > /tmp/loo291-iteration12/result/shell-returned.txt",enter:true)
            settle(1)
            ...
            try press(root,title:"Work details")
            try check(elements(root).contains{text($0,"AXIdentifier")=="workspace-task-directive"},"Task directive survives disposable Session completion")
            let remaining = try sessions()
            try check(remaining.contains { $0["id"] as? String == "run_643d86d70d5b4109a460e80a87a97533" }, "Existing human Session remains unresolved")
