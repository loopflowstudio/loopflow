// Production-like rejection trial: real reads; PM writes intercepted by an isolated transport. No provider actions.
import AppKit
...
func find(_ root: AXUIElement, id: String) throws -> AXUIElement {
    ...
    while !elements(root).contains(where: { text($0, "AXIdentifier") == id }) && ProcessInfo.processInfo.systemUptime < deadline { settle(0.1) }
...
setbuf(stdout, nil)
let proofRoot = "/tmp/loo291-review13-rejection/result"
...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: "/tmp/loo291-review13-rejection/Loopflow Rejection.app"), configuration: configuration) { running, error in
    ...
    DispatchQueue.main.asyncAfter(deadline:.now()+1) {
        ...
        do {
            ...
            try press(root,title:"Save directive")
            let rejection=try find(root,id:"task-directive-error")
            try check(text(rejection,"AXValue").contains("Review transport rejected") || text(rejection,"AXDescription").contains("Review transport rejected"),"Visible transport rejection")
            try check(text(try find(root,id:"task-directive-draft"),"AXValue")==draft,"Exact draft retained after rejection")
            settle(16)
            try check(text(try find(root,id:"task-directive-draft"),"AXValue")==draft,"Exact draft survives polling interval")
            _ = try find(root,id:"task-directive-error")
            try press(root,title:"Save directive")
            _ = try find(root,id:"task-directive-error")
            try check(text(try find(root,id:"task-directive-draft"),"AXValue")==draft,"Retry preserves exact rejected draft")
            try check(try directive()==original,"Shared directive unchanged after rejected saves")
