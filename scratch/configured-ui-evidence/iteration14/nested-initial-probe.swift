...
setbuf(stdout, nil)
let proofRoot = "/tmp/loo291-iteration14-ax-result"
...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: "/tmp/loo291-iteration12-editor/Loopflow Editor.app"), configuration: configuration) { running, error in
    ...
    DispatchQueue.main.asyncAfter(deadline: .now()+1) {
        ...
        do {
            ...
            try click(try byID("sessions-new-shell"),"Create main checkout shell")
            ...
            try click(try byID("sessions-new-conversation"),"Start fresh owned Task conversation")
            ...
            try click(try byID("sessions-new-shell"),"Create Task companion")
            ...
            try click(try byID("sessions-new-shell"),"Create main companion")
            ...
            let completion=try byID("session-action-complete")
            ...
            try click(try byID("session-action-complete"),"Complete only the owned local Session")
