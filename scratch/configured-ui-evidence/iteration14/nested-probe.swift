...
setbuf(stdout, nil)
let proofRoot = "/tmp/loo291-iteration14-retention-result"
...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: "/tmp/loo291-iteration14/Loopflow Nested.app"), configuration: configuration) { running, error in
    ...
    DispatchQueue.main.asyncAfter(deadline: .now()+1) {
        ...
        do {
            ...
            func waitFor(_ predicate: () -> Bool, seconds: Double = 20) -> Bool {
                let deadline=Date(timeIntervalSinceNow:seconds)
            ...
            func panes() -> [AXUIElement] { elements(root).filter { text($0,"AXIdentifier")=="pane-split-right" } }
            func shellReceipts() throws -> [String] {
                let output=String(decoding:try run("/bin/ps",["-axo","pid=,ppid=,tty=,lstart=,comm="]),as:UTF8.self)
                return output.split(separator:"\n").compactMap { line in
                    let fields=line.split(whereSeparator: {$0.isWhitespace})
                    guard fields.count>=9,Int32(fields[1])==running.processIdentifier,fields[2] != "??" else { return nil }
            ...
            settle(1)
            try save(elements(root).map { ["role":text($0,"AXRole"),"id":text($0,"AXIdentifier"),"label":text($0,"AXDescription"),"title":text($0,"AXTitle")] },"shell-controls.json")
            ...
            try press(root,title:"Show work list")
            func selectSession() throws {
                let targetID="session-row-"+id
                if let row=elements(root).first(where:{text($0,"AXIdentifier")==targetID}) {
                    try click(row,"Select exact Session row into originating shell")
                    return
                }
                let deadline=Date(timeIntervalSinceNow:20)
                while Date()<deadline {
                    let pickers=elements(root).filter { text($0,"AXRole")=="AXMenuButton" && (Int(text($0,"AXDescription")) != nil || Int(text($0,"AXTitle")) != nil) }
                    for picker in pickers {
                        print("SESSION_PICKER",text(picker,"AXTitle"))
                        guard AXUIElementPerformAction(picker,kAXPressAction as CFString) == .success else { continue }
                        settle(0.3)
                        print("SESSION_OPTIONS",elements(root).filter{text($0,"AXRole")=="AXMenuItem"}.map{[text($0,"AXIdentifier"),text($0,"AXTitle")]})
                        if let row=elements(root).first(where:{text($0,"AXIdentifier")==targetID}) {
                            try click(row,"Select exact Session from multiple-Session picker")
                            return
                        }
                        _ = AXUIElementPerformAction(picker,kAXCancelAction as CFString)
                    }
                    settle(0.5)
                }
                try save(elements(root).map{["role":text($0,"AXRole"),"id":text($0,"AXIdentifier"),"label":text($0,"AXDescription"),"title":text($0,"AXTitle")]},"missing-session-controls.json")
                throw Failure(message:"Exact Session action unavailable")
            }
            try selectSession()
            ...
            var pickerActions: CFArray?
            _ = AXUIElementCopyActionNames(picker,&pickerActions)
            print("WORKTREE_ACTIONS",pickerActions ?? [] as CFArray)
            let pickerAction=(pickerActions as? [String] ?? []).contains(kAXShowMenuAction) ? kAXShowMenuAction : kAXPressAction
            try check(AXUIElementPerformAction(picker,pickerAction as CFString) == .success,"Open worktree picker")
            settle()
            guard let main=elements(root).first(where:{text($0,"AXRole")=="AXMenuItem" && text($0,"AXTitle")=="loopflow"}) else {
                ...
                try save(elements(root).map{["role":text($0,"AXRole"),"id":text($0,"AXIdentifier"),"label":text($0,"AXDescription"),"title":text($0,"AXTitle")]},"missing-worktree-controls.json")
            ...
            print("PTY_RECEIPTS",initialShells)
            try save(initialShells,"shells-before.json")
            try check(initialShells.count==4,"Four direct PTY children have process receipts")
            ...
            for _ in 0..<10 {
                ...
                if text(element,"AXDescription").hasSuffix(", active") || text(element,"AXDescription").hasSuffix(", open") {
