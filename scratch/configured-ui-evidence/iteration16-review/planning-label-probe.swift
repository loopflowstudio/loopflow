...
func find(_ root: AXUIElement, _ id: String) throws -> AXUIElement {
    ...
    let dump = elements(root).map { element in
        Dictionary(uniqueKeysWithValues: ["AXRole", "AXIdentifier", "AXValue", "AXTitle", "AXDescription", "AXHelp"].map { ($0, text(element, $0)) })
    }
    try JSONSerialization.data(withJSONObject: dump, options: [.prettyPrinted, .sortedKeys]).write(to: URL(fileURLWithPath: "scratch/configured-ui-evidence/iteration16-review/missing-controls.json"))
    throw ProofFailure(message: "Missing control: " + id)
}
func matching(_ root: AXUIElement, _ description: String, _ predicate: (AXUIElement) -> Bool) throws -> AXUIElement {
    let deadline = ProcessInfo.processInfo.systemUptime + 5
    repeat {
    ...
    throw ProofFailure(message: "No unique control: " + description)
}
func inspectPlanning(_ root: AXUIElement, records: [[String: Any]]) throws {
    ...
    _ = try matching(root, "selected Task Session title") {
        text($0, "AXRole") == "AXButton" && text($0, "AXDescription") == session["title"] as! String
    }
    let toggle = try matching(root, "Show work list") {
        text($0, "AXRole") == "AXButton" && text($0, "AXDescription") == "Show work list"
    }
    ...
    print("PASS Exact Session list access is present; no Session button was pressed")
    ...
    _ = try matching(root, "Project no-Session state") { text($0, "AXValue") == "No open Sessions for this Work" }
