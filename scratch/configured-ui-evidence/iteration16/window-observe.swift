...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: "/tmp/loo291-iteration16/Loopflow Proof.app"), configuration: configuration) { app, error in
    ...
    print("Console session", CGSessionCopyCurrentDictionary() as Any)
    ...
    try! data.write(to: URL(fileURLWithPath: "/Users/jack/src/loopflow.main-view-task/scratch/configured-ui-evidence/iteration16/window-controls.json"))
    print("AX nodes", rows.count)
    let windows = (CGWindowListCopyWindowInfo(.optionAll, kCGNullWindowID) as? [[String: Any]] ?? []).filter { ($0[kCGWindowOwnerPID as String] as? NSNumber)?.int32Value == app.processIdentifier }
    print("Owned WindowServer windows", windows)
    if let id = windows.first(where: { ($0[kCGWindowLayer as String] as? Int) == 0 })?[kCGWindowNumber as String] as? NSNumber {
        let capture = Process()
        capture.executableURL = URL(fileURLWithPath: "/usr/sbin/screencapture")
        capture.arguments = ["-x", "-l", id.stringValue, "/Users/jack/src/loopflow.main-view-task/scratch/configured-ui-evidence/iteration16/window.png"]
        try! capture.run(); capture.waitUntilExit()
        print("Owned screenshot status", capture.terminationStatus)
    }
    var focused: CFTypeRef?
    let focusStatus = AXUIElementCopyAttributeValue(AXUIElementCreateSystemWide(), kAXFocusedApplicationAttribute as CFString, &focused)
    var focusPID: pid_t = -1
    if let focused, CFGetTypeID(focused) == AXUIElementGetTypeID() { AXUIElementGetPid(focused as! AXUIElement, &focusPID) }
    print("System AX focus status", focusStatus.rawValue, "PID", focusPID)
