...
environment.removeValue(forKey: "LOOPFLOW_UI_TEST_MODE")
environment.removeValue(forKey: "LOOPFLOW_UI_SNAPSHOT_PATH")
...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: "/tmp/loo291-integration/Loopflow Integration.app"), configuration: configuration) { running, error in
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
        ...
        do {
            ...
            try check(running.isActive, "Owned app active before terminal input")
            let receipt = "/tmp/loo291-review7-input-" + String(running.processIdentifier)
