    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
        ...
        do {
            ...
            if let window = ownedWindows.first(where: { ($0[kCGWindowIsOnscreen as String] as? Bool) == true }), let number = window[kCGWindowNumber as String] as? Int {
                ...
                capture.arguments = ["-x", "-l", String(number), "/tmp/loo291-iteration7-palette.png"]
                try capture.run(); capture.waitUntilExit(); try check(capture.terminationStatus == 0,"Capture configured workspace palette")
            } else { throw Failure(message:"No owned onscreen window") }
        } catch { print("FAIL", error, "terminated", running.isTerminated, "active", running.isActive); exitStatus = 1 }
    }
}
