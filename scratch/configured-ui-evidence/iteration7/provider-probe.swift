// Historical single-use proof: this exact owned Session is now completed. Do not rerun.
import AppKit
...
// Independent app instance uses the installed Home, not its launching agent's historical control Home.
...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: "/tmp/loo291-review/Loopflow Review.app"), configuration: configuration) { running, error in
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
        ...
        do {
            ...
            settle()
            let ownedId = "run_9544c29c5a6747c391e31c8523d18f4e"
            ...
            func clientReceipt() throws -> String {
                let dir = URL(fileURLWithPath: "/Users/jack/.lf-dev/installed/local-be852452823d43d7b7fde663651a7590/runs/95/" + ownedId + "/provider-clients")
            ...
            settle(2)
            send("printf '%s\\n' \"$$\" > /tmp/loo291-iteration7-companion-owned.pid; while IFS= read -r proof_line; do printf '%s:%s\\n' \"$$\" \"$proof_line\" >> /tmp/loo291-iteration7-companion-replies.log; done", enter: true)
            settle(1)
            let companionPid = try String(contentsOfFile: "/tmp/loo291-iteration7-companion-owned.pid", encoding: .utf8).trimmingCharacters(in: .whitespacesAndNewlines)
            ...
            let historyPath = "/Users/jack/.lf/accounts/codex/engineering/sessions/2026/09/23/rollout-2026-09-23T18-37-14-01a0d10f-4512-79a3-8fc3-ecaab404d709.jsonl"
            ...
            while ProcessInfo.processInfo.systemUptime < deadline {
                ...
                replies = history.split(separator: "\n").filter { line in
                    ...
                    return content.contains { $0["text"] as? String == "saffron" }
                }.count
                if replies >= 1 { break }; settle(0.2)
            }
            try check(replies >= 1, "Provider replied after navigation")
            let history = try String(contentsOfFile: historyPath, encoding: .utf8)
            ...
            settle(1)
            let companionReply = try String(contentsOfFile: "/tmp/loo291-iteration7-companion-replies.log", encoding: .utf8)
