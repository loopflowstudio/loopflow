...
func elements(_ root: AXUIElement) -> [AXUIElement] {
    ...
    func walk(_ e: AXUIElement, _ depth: Int) {
        ...
        for key in ["AXWindows", "AXChildren", "AXVisibleChildren"] {
...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: "/tmp/loo291-review/Loopflow Review.app"), configuration: configuration) { running, error in
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
        ...
        do {
            ...
            let receipt = "/tmp/loo291-review6-input-" + String(running.processIdentifier)
            ...
            let bars = elements(root).filter { text($0,"AXRole") == "AXScrollBar" }
            print("SCROLLBARS",bars.count, bars.map { String(describing:value($0,"AXValue")) })
            if let bar = bars.first {
                print("SET_SCROLL", AXUIElementSetAttributeValue(bar, kAXValueAttribute as CFString, NSNumber(value:0.55)).rawValue)
                settle()
            }
            print("SCROLL_BEFORE", elements(root).filter { text($0,"AXRole") == "AXScrollBar" }.map { String(describing:value($0,"AXValue")) })
            if let window = ownedWindows.first(where: { ($0[kCGWindowIsOnscreen as String] as? Bool) == true }), let number = window[kCGWindowNumber as String] as? Int {
                let capture = Process(); capture.executableURL = URL(fileURLWithPath:"/usr/sbin/screencapture")
                capture.arguments = ["-x", "-l", String(number), "/tmp/loo291-iteration7-workspace.png"]
                try capture.run(); capture.waitUntilExit(); print("SCREENSHOT_STATUS",capture.terminationStatus)
            }
            for candidate in elements(root).filter({ text($0,"AXIdentifier") == "podium-repo-scope" }) {
                var names: CFArray?; AXUIElementCopyActionNames(candidate,&names)
                print("MENU_ACTIONS",names as Any)
            }
            let menu = try find(root,id:"podium-repo-scope")
            try check(AXUIElementPerformAction(menu,kAXShowMenuAction as CFString) == .success,"Open repository menu")
            settle()
            print("MENU",elements(root).filter { text($0,"AXRole").contains("Menu") }.map { text($0,"AXRole") + ":" + text($0,"AXTitle") + ":" + text($0,"AXDescription") })
            func selectRepo(_ title: String) throws {
                guard let entry = elements(root).first(where: { text($0,"AXRole") == "AXMenuItem" && text($0,"AXTitle") == title }) else { throw Failure(message:"Missing repository " + title) }
                try check(AXUIElementPerformAction(entry,kAXPressAction as CFString) == .success,"Select repository " + title)
                settle(2)
            }
            try selectRepo("kata")
            let otherMenu = try find(root,id:"podium-repo-scope")
            try check(AXUIElementPerformAction(otherMenu,kAXShowMenuAction as CFString) == .success,"Open repository menu in other scope")
            settle()
            try selectRepo("loopflow")
            print("SCROLL_AFTER", elements(root).filter { text($0,"AXRole") == "AXScrollBar" }.map { String(describing:value($0,"AXValue")) })
            try press(root, title: "Work details")
            try check(elements(root).contains { text($0,"AXValue").contains("In the existing desktop workspace") },"Selected Task survives repository return")
