...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: appPath), configuration: configuration) { running, error in
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
        ...
        do {
            ...
            while ProcessInfo.processInfo.systemUptime < deadline {
                observed = elements(root).contains { text($0,"AXIdentifier") == "podium-sessions" && text($0,"AXValue") == expected }
                if observed { break }; settle(0.025)
            }
            print("COUNT_ENDPOINT", expected, "observed", observed, "milliseconds", (ProcessInfo.processInfo.systemUptime - launchedAt) * 1000)
            try check(observed,"Correct scoped Session count visible")
