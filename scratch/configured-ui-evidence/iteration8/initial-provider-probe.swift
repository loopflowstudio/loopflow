...
let proofRoot = "/tmp/loo291-iteration8"
...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: proofRoot + "/Loopflow Proof.app"), configuration: configuration) { running, error in
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
        ...
        do {
            ...
            let sentence="Do not use tools or edit files. Reply only LOO291_ITER8_CONFIRMED."
            ...
            try send("Reply only LOO291_ITER8_CONFIRMED.",enter:true)
            let replyDeadline=ProcessInfo.processInfo.systemUptime+60
            while try !messages("assistant").contains("LOO291_ITER8_CONFIRMED") && ProcessInfo.processInfo.systemUptime<replyDeadline { settle(0.3) }
            let user=try messages("user").last ?? ""
            let answered=try messages("assistant").contains("LOO291_ITER8_CONFIRMED")
            ...
            try send("printf 'LOO291_SHELL_RETURNED\\n' > /tmp/loo291-iteration8/shell-returned.txt",enter:true)
            settle(1)
            ...
            print("PASS configured New conversation, exact draft, completion and shell survival")
