...
let proofRoot = "/tmp/loo291-iteration9/provider"
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
        ...
        do {
            ...
            func send(_ input:String,enter:Bool=false) throws {
                ...
                print("Foreground identity",NSWorkspace.shared.frontmostApplication?.processIdentifier ?? -1,"owned",running.processIdentifier,"cached active",running.isActive)
                try check(NSWorkspace.shared.frontmostApplication?.processIdentifier == running.processIdentifier,"Owned app foreground for input")
            ...
            settle(1)
            try send("printf '%s\\n' $$ > /tmp/loo291-iteration9/provider/main-shell.pid",enter:true)
            ...
            print("Foreground identity",NSWorkspace.shared.frontmostApplication?.processIdentifier ?? -1,"owned",running.processIdentifier,"cached active",running.isActive)
            try check(NSWorkspace.shared.frontmostApplication?.processIdentifier == running.processIdentifier,"Owned app foreground before launch")
            ...
            try press(root,title:"New terminal")
            settle(1)
            try send("printf '%s\\n' $$ > /tmp/loo291-iteration9/provider/task-companion.pid",enter:true)
            ...
            try press(root,title:"New terminal")
            settle(1)
            try send("printf '%s\\n' $$ > /tmp/loo291-iteration9/provider/main-companion.pid",enter:true)
            ...
            let sentence="Do not use tools or edit files. Reply only LOO291_ITER9_ROW_RETURN."
            ...
            try send("Reply only LOO291_ITER9_ROW_RETURN.",enter:true)
            let replyDeadline=ProcessInfo.processInfo.systemUptime+60
            while try !messages("assistant").contains("LOO291_ITER9_ROW_RETURN") && ProcessInfo.processInfo.systemUptime<replyDeadline { settle(0.3) }
            let user=try messages("user").last ?? ""
            let answered=try messages("assistant").contains("LOO291_ITER9_ROW_RETURN")
            ...
            try send("printf 'LOO291_SHELL_RETURNED\\n' > /tmp/loo291-iteration9/provider/shell-returned.txt",enter:true)
            settle(1)
