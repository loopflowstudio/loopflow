...
let proofRoot = "/tmp/loo291-review9-provider"
...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: "/tmp/loo291-iteration8/Loopflow Proof.app"), configuration: configuration) { running, error in
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
        ...
        do {
            ...
            settle(1)
            print("Active after request", running.isActive, "AX focused PID", focusedPID())
            let console = CGSessionCopyCurrentDictionary() as? [String:Any] ?? [:]
            print("Console locked", console["CGSSessionScreenIsLocked"] ?? "absent", "login done", console["kCGSessionLoginDoneKey"] ?? "absent")
            try check(focusedPID() == running.processIdentifier,"Owned app has exact AX focus before any UI mutation")
            ...
            settle(1)
            try send("printf '%s\\n' $$ > /tmp/loo291-review9-provider/main-shell.pid",enter:true)
            ...
            try press(root,title:"New terminal")
            settle(1)
            try send("printf '%s\\n' $$ > /tmp/loo291-review9-provider/task-companion.pid",enter:true)
            ...
            try press(root,title:"New terminal")
            settle(1)
            try send("printf '%s\\n' $$ > /tmp/loo291-review9-provider/main-companion.pid",enter:true)
            ...
            let sentence="Do not use tools or edit files. Reply only LOO291_REVIEW9_ROW_RETURN."
            ...
            try send("Reply only LOO291_REVIEW9_ROW_RETURN.",enter:true)
            let replyDeadline=ProcessInfo.processInfo.systemUptime+60
            while try !messages("assistant").contains("LOO291_REVIEW9_ROW_RETURN") && ProcessInfo.processInfo.systemUptime<replyDeadline { settle(0.3) }
            let user=try messages("user").last ?? ""
            let answered=try messages("assistant").contains("LOO291_REVIEW9_ROW_RETURN")
            ...
            try send("printf 'LOO291_SHELL_RETURNED\\n' > /tmp/loo291-review9-provider/shell-returned.txt",enter:true)
            settle(1)
