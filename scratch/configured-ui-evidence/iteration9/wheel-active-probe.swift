...
NSWorkspace.shared.openApplication(at: URL(fileURLWithPath: "/tmp/loo291-iteration8/Loopflow Proof.app"), configuration: configuration) { running, error in
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
        ...
        do {
            ...
            _ = try run("/usr/sbin/screencapture",["-x","-l",number.stringValue,proofRoot+"/wheel-bottom.png"])
            let point=CGPoint(x:bounds["X"]!+bounds["Width"]!*0.7,y:bounds["Y"]!+bounds["Height"]!*0.7)
            print("Wheel activation",running.activate(options:[.activateAllWindows])); settle(0.5)
            try check(running.isActive,"Owned app active before wheel")
            let oldPointer=CGEvent(source:nil)!.location
            defer { CGEvent(mouseEventSource:nil,mouseType:.mouseMoved,mouseCursorPosition:oldPointer,mouseButton:.left)!.post(tap:.cghidEventTap) }
            ...
            settle(1)
            _ = try run("/usr/sbin/screencapture",["-x","-l",number.stringValue,proofRoot+"/wheel-scrolled.png"])
            ...
            _ = try run("/usr/sbin/screencapture",["-x","-l",number.stringValue,proofRoot+"/wheel-returned.png"])
