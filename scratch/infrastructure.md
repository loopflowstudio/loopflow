...
happens to parse.
ok, i want to address the fact that i find the product unusuable even though it now kinda sort starts to work. Herea re the main blockers I
  think:

  (1) it sometimes seems slow to output its tokens, ike only a cuple words a second or something. we need to spin up the performance
  optimization team

# 5 Whys: An unreleased migration made the installed `lf` reject its live store

## The Problem

The installed `lf 0.11.1` could not open `~/.lf/loopflow.db` after Context Lab
advanced it beyond the migration chain known to the installed release.

## Chain

Installed `lf` rejects `0.11.008_context_pressure` → an in-flight Context Lab
binary advanced the production store → Context Lab was pointed at the real
ledger → the break-glass path coupled live-data access to migration authority →
migration authority was inferred from environment/build placement instead of an
explicit release boundary

**Problem**: `lf status` and every other store-backed command warned that
`0.11.008_context_pressure` was unknown; the database had already reached
`0.11.009_context_input_normalization`, while installed `lf` and `main` knew only
through `0.11.007_task_pr_parent`.
↳ *Could we have caught this earlier?* `lf doctor` exposed both ids, but nothing
barred the schema advance before the older release became unusable.

The mechanism repeated during the incident. At 14:40 PDT an auth-compat release
binary replaced the temporary Context Lab install and advanced the same live
store to `0.11.010_profiles` and `0.11.011_provider_account_lifecycle`. Those
migrations are committed on their feature branch, but still absent from
`main`. This is confirming evidence for the root cause, not a second cause.

**Why 1**: Migrations `0.11.008` and `0.11.009` existed only in the dirty Context
Lab worktree when a Context Lab binary applied them to `~/.lf/loopflow.db`.
↳ *What process allowed this?* The migrations were executable before their
identities were reserved in the release chain on `main`.

**Why 2**: Context Lab needed the machine's real run/context evidence, while its
isolated development store had an older divergent `0.11.004_context_launch_work`
history. The dev-app path therefore carried `LF_DB_PATH=~/.lf/loopflow.db` and
`LF_ALLOW_PRODUCTION_DB_FROM_DEV=1`.
↳ *What assumption was wrong?* “Point this dev build at real data” was treated as
equivalent to “let this dev build upgrade the real schema.”

**Why 3**: The break-glass flag disabled the production-store guard completely;
`SqliteStore::new` always ran every pending migration before serving any read or
write.
↳ *Why was that assumption encoded?* Store selection and migration authority
were one decision in the open path.

**Why 4**: Release packaging also wrote a release-trusted binary to
`target/release/lf`, the same path the dev launcher treated as a source-tree
candidate. A packaged build could therefore retain production authority after
being reused as a development binary.
↳ *What boundary was missing?* Build provenance described how an artifact was
compiled, not where it was later executed; the artifact paths did not preserve
that distinction.

**Why 5 (Root)**: Schema promotion is not yet a first-class release operation.
An environment flag or reusable build artifact can grant migration authority,
and the shared store has no rule that an unreleased branch may use current
production data but may not advance production schema.

## Unanswered Whys

| Branch Point | Unexplored Question | Priority |
|--------------|---------------------|----------|
| Why 1 | Which exact Context Lab process applied each row? `schema_migrations` records time and identity, not build/process provenance. | Medium |
| Why 2 | Should Context Lab read a live current-schema store or an automatic SQLite snapshot when its schema is in flight? | High |
| Why 4 | Which other release scripts leave trusted binaries in paths later treated as dev candidates? | Medium |

## Fixes

| Level | Fix | Prevents |
|-------|-----|----------|
| Immediate | Preserve the stale installed binary, then install the exact Context Lab binary whose known chain matches live `0.11.009`. | Current CLI outage without rolling back live Task writes |
| Structural | On production, development binaries validate an already-current chain and refuse pending migrations even with break glass. | Dev inspection/dogfood silently advancing the live schema |
| Structural | Build install/package release artifacts under `target/distribution`, never the dev launcher's `target/release/lf`. | Release provenance leaking through a reused source-tree artifact |
| Systemic | Reserve `0.11.008` through `0.11.011` on `main` now that a live database carries them; only release binaries may advance production. | Older current releases rejecting a shared live ledger after branch work |
| Systemic | Record the applying build/process with future migration ledger entries or an adjacent audit event. | An incident whose writer can only be inferred after the fact |

## Changes to Implement

- [x] Restore the installed CLI with the exact schema-matching binary; preserve
  the prior executable as `~/.local/bin/lf.pre-context-009`.
- [x] Make the development production-store path validation-only when its known
  chain differs from the applied chain.
- [x] Separate distribution Cargo artifacts from the dev launcher's candidate
  path and cover the install/release scripts with tests.
- [ ] Land the already-applied `0.11.008_context_pressure` through
  `0.11.011_provider_account_lifecycle` identities into the canonical migration
  chain before another main build replaces the temporary installed binary.
- [ ] Add applying build/process provenance to the migration audit trail.

# Rebase resolver auth failure

`lf rebase` selected a managed Claude profile whose registry row said
`connected`, but the first provider request failed because its OAuth refresh
session had expired. Claude 2.1.210 made the preflight deceptive:
`claude auth status` reported the isolated profile as active even though the
request failed, and after importing the current macOS Keychain blob into the
profile it reported `none`. Supplying the same current access token through
`CLAUDE_CODE_OAUTH_TOKEN` succeeded while preserving `CLAUDE_CONFIG_DIR` for
session isolation.

The durable account-router fix belongs on the auth-compat task after its active
rebase settles:

- validate the extracted token and its expiry, not `claude auth status`;
- launch a native Claude profile with both its config directory and explicit
  OAuth token;
- mark an unusable account `missing`, fall through to the next profile, and
  print the exact reconnect command if none remain;
- seed trust for an explicitly routed repository or surface it as a profile
  setup requirement.

The local unblock is a narrow Claude launcher shim at `~/.local/bin/claude`;
the original symlink is preserved as `claude.loopflow-original-link`. It adds
the explicit token only for `~/.lf/accounts/claude/*` profiles. A live
headless `lf` launch now succeeds through the same route that failed in
`lf rebase`.
