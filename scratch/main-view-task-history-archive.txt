# Historical design and evidence — through 2026-09-25

Superseded as implementation direction by [the current design](main-view-task.md).
The original text below is retained intact for provenance and prior proof limits.
Dated proposals are not current requirements.

# One workspace for work and Sessions

## Accepted visual direction — 2026-09-25

**A calmer desktop workspace** is the guiding phrase. The human said it is
“a good vibe for this project” and selected A: “A seems like the right default
view, and you can zoom > Session with Context > Session”.

Build one continuous workspace with three reversible presentation depths:

1. **Overview (default A):** quiet Work outline, readable Task brief, existing
   Session and retained companion/Monitor panes.
2. **Session with context:** foreground the selected Session while keeping useful
   Task context nearby.
3. **Session:** foreground that same Session with a minimal way back.

Accepted: the default composition and zoom sequence. The revised web study tests
one interpretation of the middle depth: a compact Task context with outline and
companion panes temporarily hidden. Exact context placement, controls and keyboard
behavior are proposed details, not separately accepted decisions.

The human also asked to simplify the ELSEWHERE tag. Remove location badges from
conversation rows; surface a necessary explicit opening/transfer choice only in
the selected conversation. This removes list noise without inferring attachment
or authorizing an automatic client transfer.

The repository scopes the default workspace. Put the current repo in the app
header and flow directly into its Waves/Tasks in navigation; omit the repeated
repo root while scoped. The header and sidebar form one connected visual frame.
Search is anchored at the bottom so the data connects to the repo heading above.
Switching repos is explicit in the header, preserving each repo's place and
workspace. A broader All repositories choice is a study proposal, not a second
permanent navigation tree.

Zoom changes presentation only. It preserves exact Session identity, draft,
viewport, pane arrangement and running companions. Zooming out restores them.
No-Session and elsewhere states stay explicit; zoom cannot create or move a client.
The native implementation must use the existing
multiplexer and native surface owner, not three workspace trees or a new transcript.

### Task entry experiment — 2026-09-25

Task page header: show the Task title once in the top content row. Put the issue
identifier after the parent Wave in the toolbar breadcrumb, linking to Linear. Remove the generic Open
label, oversized duplicate title and separate Open in Linear action. Internal
preview diagnostics (such as missing Session attribution) do not belong in the
main Task content. Missing evidence still must not create invented counts.
Use the existing sans-serif font for the Task title; the human disliked serif
in this Task-page context. Wave heading typography is unchanged by this feedback.
Label the Task's Linear description **Description**, not Directive.
Keep comments collapsed by default. Use comments for dated planning/execution
updates and the description for the current brief, including material blockers.
The captured chapter allocation remains description text until an authorized
source edit; presentation must not silently relabel it as a comment.
The human selected header treatment A, then refined it: clickable parent Wave
and linked issue ID in the toolbar breadcrumb; full Task title only in the content
heading, without repeating the issue ID alongside.
This is now the website default; B/C remain explicit comparison options.
Remove the “Linear snapshot · read only” preview label.

The latest content review uses current Linear-backed reads for Loopflow, Etude
and Kata instead of illustrative Task names and directives. Preserve provider
text exactly and render Markdown; Wave objectives retain their shared ownership.
The current-data website is read-only and labels missing chapter/Session evidence.
The earlier 50-Task population remains an explicit density fixture, not real data.

Wave overview composition: retain the objective as displayed (the human said
“The objective is great as it is”), followed by Current KRs, then the full Task
plan. KRs and their targets come from the current chapter's internal Project,
presented through the Wave. The website uses illustrative KRs without claiming
measured progress; this is not an amendment to live planning.

Latest human clarification: the sidebar contains only Tasks whose work has
started, whether or not a Session is open. Click a Wave to see and work with its
full plan, including unstarted Tasks, in the middle pane. There is no separate
All tasks shortcut in the sidebar. Merely inspecting an unstarted Task does not
add it to the working list. Session count and started-work membership are
independent; the native implementation must use shared started-work evidence,
not infer it from Session presence or a presentation status label.

The human is reconsidering visible Task → Session nesting: “maybe we have an
in-line indicator or count for open sessions” and clicking a Task with one open
Session should open that Session. The next A study replaces Session child rows
with a quiet count on the Task. Open means unresolved conversation, independent
of whether its provider is currently running or attached here.

Proposed mechanics to test: one local Session enters Session with context; zero
opens Task Overview; several opens Overview with the exact conversation picker.
The count opens Overview so the Task brief remains directly reachable even with
one Session. An external conversation keeps its explicit opening choice and is
never automatically transferred. Unbound conversations remain reachable. Initial
A remains Overview, and zoom/back preserves the same conversation and workspace.
These fallback and count-click details are prototype proposals, not separately
accepted decisions. Native navigation has not changed.

This slice refines [A in the study](visual-study/index.html?v=a). B and C remain
comparison history. Finish the zoom interaction and get feedback before applying
the composition to the installed app. Native proof must later exercise actual
focus, draft, scroll and companion retention across every depth.

## Earlier visual review — 2026-09-25

The human rejected the restored demo: “it still looks kinda horrible tbh”.
Restored chapter data, a burgundy/deep-wine outline, serif Wave headings and
passing native/reader checks do not establish a successful composition.
The color restoration was insufficient. Human visual acceptance remains open.

Observed in the configured capture: long Task names wrap densely in the rail;
unmatched Sessions add repeated ancestry text; the inspector foregrounds technical
status and displays the long directive as a wall of largely unformatted text.
The earlier reference screens used a clearer burgundy frame, cream working
surface and stronger division between navigation and the selected workspace.
These are visual observations, not a diagnosis of which aspect the human dislikes.

The human requested three interactive website mockups and research into Notion,
Figma and Claude Desktop, including missing affordances. The
[research and source audit](research-workspace-design-603c72da.md) separates existing
behavior from presentation gaps. The [comparison website](visual-study/index.html)
hosts three compositions authored by Opus 5.5 through `lf -m claude`.
The study passes local interaction checks at desktop and laptop widths; it is
sample-data design evidence, with human preference still open.
A was subsequently selected; the accepted direction above governs the next refinement.
Preserve the accepted one-outline hierarchy and existing multiplexer. No new
navigation model, chapter rotation or provider action follows from this feedback.
