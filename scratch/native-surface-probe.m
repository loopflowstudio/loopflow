...
int main(int argc, char **argv) { @autoreleasepool {
    ...
    CVReturn linkStatus = CVDisplayLinkCreateWithActiveCGDisplays(&link);
    printf("displayLinkStatus=%d created=%s\n", linkStatus, link ? "true" : "false");
    ...
    if (argc > 1) ghostty_config_load_file(cfg, argv[1]);
    ...
    printf("surface=%s config=%s\n", surface ? "created" : "nil", argc > 1 ? "explicit" : "omitted");
