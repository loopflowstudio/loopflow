...
- Credential mounts in `lfd.yaml` are parsed as explicit `host_path:container_path` strings. We should confirm whether we want first-class structured mount config (plus `~` expansion) before making this user-facing.
