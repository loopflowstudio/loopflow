...
- Local macOS UI validation still fails in `xcodebuild test` with `ConcertoUITests-Runner` crashing before the test runner bootstraps. Swift package tests pass, so this currently looks isolated to the UI-test harness rather than the journal changes in this branch.
