...
| Lock bypass is receipt-scoped | Only the active switch recovery may launch a replacement while promotion holds the exclusive lock | The handoff capability carries the switch id, matches the ordinary switch capability and active receipt, and then proves exclusive ownership | Source inspection plus the public target-restart path | Pass |
