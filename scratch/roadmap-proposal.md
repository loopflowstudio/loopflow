...
### Stage B: Rename `roadmap/` → `wave/`

The plan and the execution config are the same thing. `roadmap/` implies a document you read. `wave/` implies something you run.

- Rename `roadmap/` to `wave/` across the codebase
- Update all prompts, docs, and code that reference `roadmap/`
- Each file in `wave/` is both the plan and the wave spec

### Stage C: Wave Specs & Launcher

Each `wave/` item declares its execution config in frontmatter:
...
- Rust: `GET /waves/launchable` endpoint (parse frontmatter, cross-reference active waves)
- Swift/Concerto: Wave launcher view (checkboxes, batch launch, planned/running/done status)
