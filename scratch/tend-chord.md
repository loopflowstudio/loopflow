...
**After**: Change `clear-the-deck.yaml` from `flow: ship-wave` to `flow: reorg` until the branch is back in sync with the on-disk roadmap and the leftover `01`/`02` phase debris is gone.
**Rationale**: This wave is not blocked on missing ideas; it is blocked on coherence. `reorg` matches the actual job now: reconcile the wave to its current roadmap, shrink the diff, and stop competing for review bandwidth while the shared `lfd`/`python` area is unstable.
**Risk**: Useful cleanup work pauses for a cycle, and `reorg` may expose that some cleanup should be deleted outright rather than carried forward.
