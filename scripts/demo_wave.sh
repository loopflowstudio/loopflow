...
command -v codex >/dev/null || { echo "missing: codex CLI (the mind runs on codex app-server)" >&2; exit 1; }
...
say 'POST /messages {"op":"message"} — queued; the mind answers at the next boundary'
...
if [[ -z "$(new_codex_orphans)" ]]; then
    ok "no new codex processes beyond the mind's own (vendor session survives an interrupt)"
fi
pause

# ---------- act 5: dispatch a worker ----------------------------------------

hr "act 5 · the mind dispatches a worker (lf q worker run)"
...
if journal_has run_observed; then
    ...
    warn "no dispatch observed — read the mind's reply above and its tmux pane"
fi
pause
...
say "from:\"worker\" byline and wakes the mind; watch for memory_updated when"
say "the mind curates what it learned (lf memory add)."
...
if journal_has memory_updated; then
    ...
    warn "no memory_updated yet — curation is the mind's judgment call, not a scripted step"
fi
pause
