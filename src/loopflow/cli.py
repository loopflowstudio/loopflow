...
def doctor():
    ...
    if not ide or ide.warp:
        if shutil.which("warp"):
            ...
            typer.echo("✗ warp - Run: lf install")
            all_ok = False
    ...
    if not ide or ide.cursor:
        if shutil.which("cursor"):
            ...
            typer.echo("✗ cursor - Run: lf install")
            all_ok = False
...
def _find_workspace(repo_root: Path, config: Config | None) -> Path | None:
    """Find workspace file to open. Returns None if should open folder instead."""
    # Explicit workspace path in config takes precedence
    ...
    # Look for exactly one .code-workspace file at repo root
...
def start(
    ...
    """Create a worktree and branch for a new feature, open IDEs."""
    repo_root = find_repo_root()
    if not repo_root:
        typer.echo("Error: Not in a git repository", err=True)
        raise typer.Exit(1)
