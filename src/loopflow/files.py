...
def is_ignored(path: Path, repo_root: Path) -> bool:
    ...
    Excludes .git, .lf (prompt config), and gitignored paths.
    """
...
def gather_docs(path: Path, repo_root: Path) -> list[tuple[Path, str]]:
    ...
    while current >= repo_root:
        ...
        for md_file in sorted(current.glob("*.md")):
            if md_file.is_file() and not is_ignored(md_file, repo_root):
...
def gather_file(path: Path, repo_root: Path) -> tuple[Path, str] | None:
    """Gather a single file if it exists and isn't gitignored."""
    if not path.exists():
        return None
    if not path.is_file():
        return None
