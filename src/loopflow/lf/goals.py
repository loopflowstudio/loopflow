"""Goal file loading for agent judgment and perspective."""
...
class Goal:
    ...
    Goals define *how* to approach work (judgment, style, perspective).
...
def parse_goal_arg(goal_arg: str | None) -> list[str]:
    ...
    if not goal_arg:
        return []
    return [v.strip() for v in goal_arg.split(",") if v.strip()]
