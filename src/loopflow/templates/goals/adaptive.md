...
- `.docs/` — vision, architecture, direction
- `.docs/roadmap/` — all planned work across all areas
- `.docs/roadmap/<your-area>/` — items you own
...
Check `.docs/roadmap/<area>/` for approved items. If there are approved items, pick the one with the most leverage—where's the biggest gap between vision and implementation?

If the roadmap is thin or unclear, switch to roadmap mode: read `.docs/` to understand direction, be honest about what's working and what isn't, then propose concrete items to `.docs/roadmap/<area>/`.
