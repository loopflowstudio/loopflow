...
struct WaveModelTests {
    ...
    func elapsedTimeSeconds() {
        let wave = Wave(
            id: "test",
            repo: "/tmp",
            runStartedAt: Date().addingTimeInterval(-45)
        )

        #expect(wave.elapsedTime() == "45s")
    }

    @Test("elapsedTime formats minutes and seconds")
    func elapsedTimeMinutesSeconds() {
        let wave = Wave(
            id: "test",
            repo: "/tmp",
            runStartedAt: Date().addingTimeInterval(-192) // 3m 12s
        )

        #expect(wave.elapsedTime() == "3m 12s")
    }

    @Test("elapsedTime formats hours and minutes")
    func elapsedTimeHoursMinutes() {
        let wave = Wave(
            id: "test",
            repo: "/tmp",
            runStartedAt: Date().addingTimeInterval(-3900) // 1h 5m
        )

        #expect(wave.elapsedTime() == "1h 5m")
    }

    @Test("elapsedTime returns nil when runStartedAt is nil")
    func elapsedTimeNil() {
        let wave = Wave(id: "test", repo: "/tmp", runStartedAt: nil)

        #expect(wave.elapsedTime() == nil)
    }

    @Test("progressDisplay combines progress and elapsed time")
    func progressDisplayCombined() {
        let now = Date()
        let wave = Wave(
            id: "test",
            flow: "ship",
        ...
        #expect(wave.progressDisplay(now: now) == "implement (2/4) · 3m 12s")
    }

    @Test("progressDisplay shows progress only when no elapsed time")
    func progressDisplayNoElapsedTime() {
        let wave = Wave(
            id: "test",
            flow: "ship",
            repo: "/tmp",
        ...
        #expect(wave.progressDisplay() == "implement (2/4)")
    }
}
