// Service for loading step execution history from lfd.db.
...
public struct SessionService: @unchecked Sendable {
    ...
    public func history(for worktree: String, limit: Int = 20) async throws -> [StepRun] {
        return loadStepRuns(worktree: worktree, limit: limit)
    }

    public func historyForRepo(_ repo: String, limit: Int = 50) async throws -> [StepRun] {
        return loadStepRuns(repo: repo, limit: limit)
    }

    private func loadStepRuns(worktree: String? = nil, repo: String? = nil, limit: Int = 20) -> [StepRun] {
        var runs: [StepRun] = []
        ...
        // Note: Database column is still 'task' for compatibility
