...
enum LocalWaveAgentLauncher {
    ...
    static func resolvedTaskReviewCommand(repoPath: String, taskId: String) throws -> [String] {
        let origin = WaveOrigin.resolve(repoPath)
        let lfPath = try resolveWaveCapableLf(originRepo: origin)
        return taskReviewCommand(lfPath: lfPath, taskId: taskId)
    }
    ...
    static func taskReviewCommand(lfPath: String, taskId: String) -> [String] {
        [lfPath, "work", "review", "task", taskId, "--continue-on-exit"]
    }
