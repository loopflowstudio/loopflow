...
struct WavePlanParserTests {
    ...
    func parsesObjectiveAndProjects() throws {
        ...
        #expect(plan.projects[0].krs.map(\.text) == [
            "A running wave session survives app restart and reattaches cleanly in 5/5 dogfood trials.",
            "Launching or attaching takes one action.",
        ])
        #expect(plan.projects[0].krs.allSatisfy { $0.proof == .open })
    }

    @Test("parses checked KR proof state")
    func parsesKRProofState() throws {
        let repoRoot = try makeTempRepo()
        defer { try? FileManager.default.removeItem(at: repoRoot) }
        let projectsDir = repoRoot
            .appendingPathComponent("wave/loopflow/projects", isDirectory: true)
        try FileManager.default.createDirectory(at: projectsDir, withIntermediateDirectories: true)
        try """
        # Runtime
        ...
        - [x] Detached loops survive the caller.
        - [ ] Foreground loops renew the pass lease.
        """.write(
            to: projectsDir.appendingPathComponent("runtime.md"),
            atomically: true,
            encoding: .utf8
        )

        let plan = try #require(WavePlanParser.parse(repoRoot: repoRoot, waveName: "loopflow"))
        #expect(plan.projects[0].krs.map(\.proof) == [.holds, .open])
        #expect(plan.projects[0].krs.map(\.text) == [
            "Detached loops survive the caller.",
            "Foreground loops renew the pass lease.",
        ])
    }
