...
The runtime model is also more concurrent than the current surface. A wave can have many runs and terminal sessions even if the workspace foregrounds one. Lifecycle UI should make that multiplicity legible instead of baking in a single-run assumption.
...
3. **Multi-run clarity.** When several runs exist for one wave, show the foreground run, the rest of the stack, and which terminal or attention item belongs to which run. Do not collapse stacked or waiting runs into one ambiguous status row.
...
- Multiple runs for one wave stay legible in the UI
