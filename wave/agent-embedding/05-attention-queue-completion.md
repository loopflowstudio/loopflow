---
linear_id: a98be1e6-820f-4fa7-b9b9-99840843ad8f
---
# Attention Queue Completion

**Finish line:** Design review and calibration checkpoints surface as typed attention items with queue-specific detail and actions, so the attention queue covers every human decision in build and garden flows.
...
The remaining gap is coverage. `design_review` and `calibration` are modeled in Rust and Swift, but they still fall back to raw context and never get created by the executor or garden flow. Until those two paths are real, the queue cannot fully replace drilling into individual waves.
...
2. **Calibration attention creation.** Wire `wave/mutate` into `calibration` attention items that capture the source assessment, applied mutations, and any human notes that should feed later garden cycles.
...
- `wave/mutate` produces `calibration` attention items with mutation context and review actions
...
- A conductor can handle build and garden checkpoints from the queue without opening a wave detail first
