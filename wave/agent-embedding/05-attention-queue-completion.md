...
2. **Calibration attention creation.** Wire `tend/play-chord` into `calibration` attention items that capture the source assessment, applied mutations, and any human notes that should feed later tend cycles.
...
- `tend/play-chord` produces `calibration` attention items with mutation context and review actions
