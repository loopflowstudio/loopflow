...
- **Runtime drift (reduced).** `lf` CLI and `/v0/sessions` HTTP use separate execution paths through the same harnesses. Prompt drift is eliminated (shared `engine/launch.rs`). Arg-building drift is mostly eliminated — `ClaudeArgs`, `build_claude_session_turn_args()`, and `build_codex_thread_start_params()` now live in `engine/agent` and are shared by both paths. Remaining gap: the session harness trait (`Harness`) still lives in `lfd/sessions/harness/`; extracting it into `engine/` is the next runtime convergence step. All three harnesses (Codex, Claude, OpenCode) are implemented; conformance replay tests (Claude normal/crash/multi-tool, Codex normal/error) validate harness behavior against recorded traces. OpenCode conformance traces still needed.
