...
Sprint 02 shipped API key auth with CLI-only key entry (`lfq auth configure <provider> --credential apikey`). The `AuthProviderCard` shows a credential type badge, but there's no UI for entering or switching keys. Users without terminal access can't opt into API key auth.
