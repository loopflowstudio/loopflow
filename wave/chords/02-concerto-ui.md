...
Signal cleanup has landed — `wave.mode` (Loop/Cron/Manual) is on the wave struct, which affects how waves display in the sidebar. FlowRun container (01) should land before or alongside this — it adds `loop_flow_run` and `triggered_flows` to WaveRun, which feeds the per-iteration detail view. Chord CRUD already works end-to-end from the HTTP API and Python client.
