...
Phases 01–03.5 collapsed `WaveRunKind`/`StimulusKind` into a single `Signal` enum. Phase 04 splits this further: execution modes (Loop, Once) move to `wave.mode`, leaving Signal as purely reactive triggers.

```
Signal (reactive, external events):    Watch | Listen | Cron | CiFailure
WaveMode (execution behavior):         Loop | Once
```
