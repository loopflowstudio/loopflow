...
- `swift/README.md` still tells users to open `Debug → Reply Demo`, so the docs will also need to match whatever product/debug decision wins.
...
3. Decide whether the demo/test windows stay debug-only or graduate into a supported product surface, then make the app registration and docs match that decision.
...
- Production Concerto builds and user-facing docs do not expose test/demo windows or dead theme paths by accident.
