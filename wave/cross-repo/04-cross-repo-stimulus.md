# 04: Cross-Repo Stimulus

## What to build

Waves can listen to waves in related repos (parents or children). The stimulus `source_repo` field references a `RepoId` instead of a raw filesystem path. The edge graph validates that the relationship exists.

## Key changes

- `Stimulus.source_repo` accepts a `RepoId` (GitHub `owner/repo`) instead of a filesystem path.
- When resolving a stimulus source, lfd checks the edge graph to confirm the repos are related.
...
2. When lfd evaluates the stimulus, it resolves the `RepoId` to a local path via the store.
...
- If the source repo isn't on disk, the stimulus is inactive (same as today when `source_repo` path doesn't exist).
