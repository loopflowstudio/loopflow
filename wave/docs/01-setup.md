# 01: Cross-Platform Init

**Finish line:** `lf init` runs on a fresh Ubuntu machine with OpenCode installed and produces a working config.

Cross-platform init, setup rationalization, platform-aware guidance. Init detects what's present — never installs. Setup entry points (lf init, lfd install, Concerto) have clear ownership and hand off cleanly.

This sprint has two deliverables — treat them as separate commits.

---

## Deliverable 1: Rewrite init.md
...
The setup routing table for getting-started.md:
```
...
Drop the flows README creation — init should create exactly one file: `.lf/config.yaml`.

### Key decisions

**Detection, never installation.** The current init runs `brew install` and `npm install`. The new init only runs `command -v` and `test`. Makes init safe to run on any platform because it can't break anything.

**No Gemini CLI.** The wave explicitly marks Gemini as "not here" — the harness doesn't exist yet. Remove all references. Add back when the harness ships.

**Drop IDE preferences.** The Warp/Cursor questions in Phase 4 are vestigial. The `IdeConfig` struct exists in config.rs but init shouldn't be asking about editors.

**Agent name without model as default.** Write `agent: claude` not `agent: claude:opus`. The engine already handles model defaults in `parse_agent()`.

**One file output.** Init creates `.lf/config.yaml` and nothing else. No flows README, no IDE config, no extra scaffolding.

**Concerto gap is out of scope.** The SetupView/SetupService in Swift also hardcode macOS/Homebrew assumptions, but fixing Concerto setup is Concerto feature work.
...
|----------|----------|---------|
| Keep init as installer, add Linux package managers | Broader platform support but init stays fragile | Init shouldn't install. Detection is reliable; installation is a moving target |
| Move init to compiled Rust code instead of a prompt | Deterministic, testable, no LLM variability | Overkill for this phase. The prompt approach works well for conversational setup |
| Auto-detect platform and install silently | Fastest path to working setup | Violates principle of least surprise. Users should consent to installations |
...
## Deliverable 2: `lf ops ingest` fast-path
