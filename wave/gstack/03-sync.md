...
- Stage 2 added flow YAML files (`.lf/flows/gstack/`) and the `synthesize` field on `and` branches. Sync must preserve manually-authored flow files — they aren't generated by the converter. The converter handles step content; flows are authored separately.
