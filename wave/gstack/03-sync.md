...
- Flow YAML files (`.lf/flows/gstack/`) and the `synthesize` field on `and` branches were added in stage 2. Sync must preserve manually-authored flow files — they aren't generated by the converter. The converter handles step content; flows are authored separately.
- Two directions were extracted: `gstack.md` (from gstack's GSTACK.md) and `openclaw.md` (from OpenClaw's SOUL.md), stored as builtin directions in `rust/loopflow/src/engine/builtins/directions/`. Sync should update these when the upstream style docs change.
- Helper binary references (`gstack-review-read`, `gstack-review-log`, `gstack-config`) remain in some imported steps. These are not loopflow-native — sync should either strip them or the converter should rewrite them to loopflow equivalents.
