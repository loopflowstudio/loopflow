...
**Finish line:** Every lfd trigger loop has at least one test. HTTP handler integration tests exist for wave and session routes. Migration integrity is tested.
...
**Migration integrity test.** Add a test that applies all 21 migrations to a fresh SQLite database and verifies the schema matches expectations. Run it in CI.
