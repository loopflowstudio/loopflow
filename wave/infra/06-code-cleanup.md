...
**Config test isolation.** `load_config_or_default_returns_defaults` fails when run from a repo with `.lf/config.yaml` (picks up `yolo: true`). The test needs to run from a tempdir or override the config search path.
