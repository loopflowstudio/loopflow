...
- **Pass 3 shipped.** Activation ingress (Milestone A) and multi-step fork branches (Milestone B) both landed. Activation is centralized with push/listen/manual coverage, queue/audit observability, and explicit drop/coalesce semantics. Fork branches support sequential multi-step flows.
