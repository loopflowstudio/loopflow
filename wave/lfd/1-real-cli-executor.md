...
The runtime journal contract (v2) is shipped and the old runtime types are deleted:
...
The old `RuntimeRun`, `RuntimeRunMeta`, `RuntimeEvent`, `RunTarget`, `map_runtime_event`, `runtime/mod.rs`, and `lfd/runtime_journal.rs` are all deleted. The journal module is the single source of execution state.
