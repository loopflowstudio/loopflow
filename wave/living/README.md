...
- Memory consolidates naturally through existing steps (consolidate, review)
...
wave/<name>/
  ...
  memory/
    SUMMARY.md          ← always fits in context, regenerated by consolidation
    codebase.md         ← architecture patterns, module roles
    patterns.md         ← what works, what doesn't, conventions learned
    preferences.md      ← user preferences, project norms observed
```

Memory enters prompt assembly through `DocumentSource::WaveMemory`. When budget is tight, `SUMMARY.md` survives and topic files are trimmed. This creates natural pressure toward concise, high-value observations.
