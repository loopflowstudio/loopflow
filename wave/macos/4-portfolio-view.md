...
The single-repo home screen now exists: `WaveWorkspaceView` routes selected waves to a workspace multiplexer. The default layout is roadmap list (left, 32%) + roadmap detail + terminal (right vertical split). Roadmap is a compact selectable list (title + priority) with a shared `RoadmapSelection` environment object driving the detail pane. `AttentionQueueView` is the no-selection home. The queue empty state shows wave overview cards — each wave with a status icon, display name, vision tagline (now parsed from the README opening paragraph, with legacy `## Vision` fallback), and diff indicator — clickable to select. This is a lightweight portfolio surface within the repo window. There is also already a repo-card portfolio shell in `PortfolioWindow`: it lists repos, shows wave counts / blocked counts / diff totals, and lets the conductor jump straight into a repo or wave. Portfolio is the next scale step — deepen both existing surfaces with the same queue, run, and terminal signals instead of replacing them.
