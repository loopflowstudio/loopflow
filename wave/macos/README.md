...
The workspace multiplexer is now in place: a recursive binary split tree per wave, persisted via `MultiplexerStore`, with roadmap list, roadmap detail, README, runs, launcher, terminal, markdown, diff, and launchpad panes. The default workspace opens into roadmap list (left, 32%) + roadmap detail + terminal (right vertical split), and the command palette focuses existing panes before creating duplicates. The terminal pane is backed by tmux (`TmuxSession`), and focus-aware keyboard routing dispatches to SwiftUI or tmux based on first-responder detection. `TerminalWorkspaceStore` manages session ordering and selection per repo; the multiplexer extends that model with spatial layout.

Roadmap is now a list/detail workspace. The list pane (`RoadmapPaneView`) shows compact selectable rows — title + priority, shipped items dimmed and struck through and sorted below unshipped, hover-to-play inline button, and `j`/`k`/arrow/`Return` keyboard navigation. Selecting an item updates the detail pane (`RoadmapDetailPaneView`) with full markdown rendering and an always-visible ingest/build action. Cross-pane state flows through a shared `RoadmapSelection` environment object so the multiplexer stays loosely coupled. Priority editing remains an inline dropdown on the priority text. Targeted ingest lives in Rust (`ops/ingest.rs`) so the CLI, daemon, and app share one implementation. Reprioritization is local-only — remote repos still need a separate edit path. Remaining composition work (directional focus, named layouts, layout migrations) should deepen these surfaces rather than introducing parallel state.

Wave sidebar taglines now parse from the README opening paragraph (text before the first blank line, skipping any `#` heading) with fallback to the legacy `## Vision` section when no leading paragraph exists.
...
- Roadmap list/detail behavior has model/parser test coverage but no dedicated UI test for keyboard navigation, selection sync, or hover interactions
