...
Roadmap panes are now actionable. Each non-shipped wave item shows an inline content summary, a priority picker (renaming files with `1-` through `4-` prefix), and a play button that calls `lf ops ingest --item <filename>` through `lfd` then runs the wave's configured flow. Targeted ingest lives in Rust (`ops/ingest.rs`) so the CLI, daemon, and app share one implementation. Reprioritization is local-only — remote repos still need a separate edit path. Remaining composition work (directional focus, named layouts, layout migrations) should deepen these surfaces rather than introducing parallel state.
