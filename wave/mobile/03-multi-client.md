...
**Status: complete** (updated February 26, 2026)
...
### ~~iOS foreground reconnect and stream restart~~ shipped (February 26, 2026)

`MobileRootView` calls `repoState.checkConnectionHealth()` on foreground resume. `MobileWaveDetailView` restarts output and session streams. `EventService.resumeFromBackground()` cancels stale backoff and retries immediately. Design: `scratch/mobile-foreground-reconnect.md`.

### ~~Cross-client suggested action clearing~~ shipped (February 26, 2026)

`SessionState.reduce()` clears suggested actions on `.turnStarted` — any client starting a turn clears stale action buttons on all connected clients.

### ~~Manual iOS visual verification script~~ shipped (February 26, 2026)

`scripts/verify_mobile_stage03.py` builds for iPhone Simulator and prints a manual checklist covering action rail spacing, foreground reconnect recovery, and cross-client action clearing.
