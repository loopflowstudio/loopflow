...
- **`lf op reset-waves`** — operator fresh-start button on the `lf op` surface: kills every `lf-*` tmux session (wave agents `lf-<repo>-<wave>` and workers `lf-<branch>-<uuid>`) and clears stale `wave/*/.wave-endpoint` pointers under the main repo. `-y/--yes` skips the confirm prompt; non-tty runs skip it automatically. tmux SIGHUPs the session's process group, so killing the `lf-` sessions takes the wave minds down too; lfd reconciles its own registry on next boot. Companion to Concerto's ghost-session reclaim — both rest on the invariant that a tmux session with no live endpoint is a ghost, not a live wave (see the concerto wave's session-lifecycle memory). Not an `lf op` *item* op — rejected by `execute_parsed_ops`.
