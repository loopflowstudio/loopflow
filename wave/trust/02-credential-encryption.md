# 02: Credential Encryption at Rest
...
Sprint 02 of the old auth wave stores API keys in the same `access_token` column as OAuth tokens, unencrypted. OAuth tokens are short-lived and auto-refresh, so exposure is time-bounded. API keys are long-lived — a leaked DB file means a leaked key.
...
- Docker executor's per-provider DB calls (3 per container launch) would each add a decrypt step. Batching + encryption should land together.
